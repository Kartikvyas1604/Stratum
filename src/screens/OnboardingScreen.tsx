...
export const OnboardingScreen: React.FC = () => {
  ...
  const onSetup = async () => {
    if (!mnemonic) {
      Alert.alert('Generate seed phrase', 'Please generate your wallet seed phrase before setup.');
      return;
    }

    if (!mnemonicBackedUp) {
      Alert.alert('Backup required', 'Confirm that you wrote down the seed phrase before continuing.');
      return;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <GlassCard>
        ...
        <Text style={styles.body}>Generate your seed phrase, write it down offline, then continue setup.</Text>
      </GlassCard>

      <PrimaryButton title={mnemonic ? 'Regenerate Seed Phrase' : 'Generate Seed Phrase'} onPress={onGenerateMnemonic} />

      {mnemonic ? (
        <GlassCard>
          <View style={styles.seedHeader}>
            <Text style={styles.seedTitle}>Recovery Seed Phrase</Text>
            <Pressable onPress={() => setShowMnemonic((value) => !value)}>
              <Text style={styles.seedToggle}>{showMnemonic ? 'Hide' : 'Show'}</Text>
            </Pressable>
          </View>

          <Text style={styles.seedBody}>
            {showMnemonic ? mnemonic : 'Tap Show to reveal your seed phrase'}
          </Text>
          <Text style={styles.warning}>Never screenshot or share this phrase. Anyone with it can access your funds.</Text>

          <Pressable style={styles.checkboxRow} onPress={() => setMnemonicBackedUp((value) => !value)}>
            <View style={[styles.checkbox, mnemonicBackedUp && styles.checkboxChecked]} />
            <Text style={styles.checkboxLabel}>I wrote down my seed phrase securely.</Text>
          </Pressable>
        </GlassCard>
      ) : null}
